<script>
  ...
  let message = "",
    randomMessages = ["Keep going", "You got this", "Awesome", "Watch out"],
    messageDis = false,
    game,
    score = 0,
    multiplier = 1;
  ...
  const sketch = (p5) => {
    ...
    p5.draw = () => {
      ...
      if (status == 1) {
        ...
        //point system
        score = Math.ceil(
          ((p5.mouseX - 39 + (p5.mouseY - 20)) * -1 + 600) / 60
        );
      }
      // Reset background
      if (
      ...
      if (
        ...
        //add and display points depending on random number and time spent
        setTimeout(function () {
          message =
            "Jackpot! You won " +
            Math.ceil(10 / ($time / 3000)) +
            " x " +
            multiplier +
            " = " +
            multiplier * Math.ceil(10 / ($time / 3000)) +
            " points";
        }, 2450);
